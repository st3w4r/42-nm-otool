...
/*   Updated: 2017/06/29 16:23:04 by ybarbier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_fromat			t_s_format;
struct							s_fromat
{
	char				*filename;
...
typedef struct s_prog			t_s_prog;
struct							s_prog
{
	uint32_t	nbfiles;
	uint32_t	flags;
...
t_s_archive_list				*add_archive_list(t_s_format *format,
									void *ar_object, char *name);

/*
** File: section_list.c
** Description: All functions to deal with the section_list
*/
t_s_section_list				*get_section_index(
									t_s_section_list *section_list,
...
** Description: All functions to deal with the section_list
*/
t_s_symbol_list					*add_symbol_list(t_s_format *format,
									void *symbol, bool is_64);
...
*/
void							display_ar_header(char *filename,
												char *name);
void							display_format(t_s_file *file,
												t_s_format *format);
...
*/
void							display_symbol_hexa_64(void *string_table,
											t_s_symbol_list *symbol_elem);
void							display_symbol_hexa_32(void *string_table,
											t_s_symbol_list *symbol_elem);
...
void							*get_ar_object(void *ptr,
												...
												size_t size_name);
void							*get_ar_string_table(void *ranlib_arr,
													size_t nranlibs);
char							*get_ar_string_element(void *string_table,
												struct ranlib *ranlib);
struct s_ar_header				*get_ar_header_element(void *ptr,
												struct ranlib *ranlib);
struct ranlib					*get_ranlib_element(void *ranlib_arr,
													size_t index);
...
void							*get_symdef(struct s_ar_header *ar_header,
								size_t size_name);
char							*get_ar_header_name(
...
*/
void							*get_object_file(void *ptr,
												uint32_t offset);
t_e_file_format					get_file_format(void *ptr);
void							*get_fat_ach(void *header,
											int index,
											bool is_64);
struct load_command				*get_first_load_command(void *header,
														bool is_64);
...
*/
void							*get_next_symbol(void *symbol_table,
												bool is_64);
void							*get_symbol_table(struct symtab_command *sym,
													void *ptr);
void							*get_string_table(struct symtab_command *sym,
													void *ptr);
char							*get_symbol_string(t_s_symbol_list *symbol_elem,
													void *string_table,
													bool is_64);
void							*get_section_command(void *seg,
...
*/
uint32_t						get_section_type(t_s_section_list *section_elem,
									bool is_64);
uint32_t						get_section_attributes(
...
void							handle_symtab_command(t_s_format *format,
						struct symtab_command *sym, void *ptr, bool is_64);
void							handle_segment_command(t_s_format *format,
											void *seg, bool is_64);
void							handle_load_command(t_s_format *format,
										struct load_command *lc,
										void *ptr, bool is_64);
void							handle_macho(t_s_format *format,
											void *ptr,
											bool is_64);
...
** File: ft_print_mem.c
*/
void							ft_print_mem(void *addr, void *memory_addr,
								int len, bool is_64);
