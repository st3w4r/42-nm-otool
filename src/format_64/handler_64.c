...
void handle_symtab_command_64(s_format *format, struct symtab_command *sym, void *ptr)
{
	uint32_t				i;
	char						*str;
	void						*string_table;
...
void handle_segment_command_64(s_format *format, struct segment_command_64 *seg, void *ptr)
{
	uint32_t i;
	...
	while (i < seg->nsects)
	{
		sec = get_section_command_64(seg, i);
...
*/
void handle_load_command_64(s_format *format, struct load_command *lc, void *ptr)
{
	...
	if (lc->cmd == LC_SYMTAB)
	{
		...
		handle_symtab_command_64(format, sym, ptr);
	}
	else if (lc->cmd == LC_SEGMENT_64)
	{
		seg = (struct segment_command_64 *)lc;
		handle_segment_command_64(format, seg, ptr);
...
void	handle_macho_64(s_format *format, void *ptr)
{
	...
	{
		// display_load_command_type(lc->cmd);
		handle_load_command_64(format, lc, ptr);
		lc = get_next_load_command(lc);
		i++;
	}
}
